  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex justify-end">
      <div className="bg-white w-full max-w-md h-full shadow-2xl flex flex-col border-l border-slate-200">
        {/* Drawer Header */}
        <div className="bg-[#1B2028] text-white p-4 flex items-center justify-between border-b border-[#2D3642] shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#DCC7AA]">Global Analytics & Policy Engine</p>
            </div>
          </div>

          <button
            onClick={onClose}
            className="p-1.5 rounded-lg bg-[#2D3642] text-slate-300 hover:text-white transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4 text-xs bg-[#FAF6EF]">
          {messages.map((msg) => (
            ...
            >
              {msg.sender === 'assistant' && (
                <div className="w-7 h-7 rounded-lg bg-[#6B7A8F] text-white flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
              ...
              <div
                className={`max-w-[85%] rounded-2xl p-3.5 space-y-2 ${
                  msg.sender === 'user'
                    ...
                    : 'bg-white text-slate-800 border border-[#DCC7AA] rounded-tl-none shadow-sm'
                }`}
              >
                ...
                {msg.suggestedQueries && (
                  <div className="pt-2 border-t border-[#DCC7AA]/60 space-y-1.5">
                    <div className="text-[10px] font-bold text-[#6B7A8F] uppercase tracking-wide">Suggested Prompts</div>
                    <div className="flex flex-wrap gap-1.5">
                      {msg.suggestedQueries.map((q, idx) => (
                        <button
                          key={idx}
                          onClick={() => handleSend(q)}
                          className="text-[11px] bg-[#FAF6EF] text-[#2D3642] hover:bg-[#F7882F]/10 border border-[#DCC7AA] px-2.5 py-1 rounded-lg text-left transition-colors font-medium"
                        >
                          {q}
                        </button>
                      ))}
                ...
                <div className={`text-[9px] text-right ${msg.sender === 'user' ? 'text-amber-100' : 'text-slate-400'}`}>{msg.timestamp}</div>
              </div>

              {msg.sender === 'user' && (
                <div className="w-7 h-7 rounded-lg bg-[#2D3642] text-white flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
          ...
          {loading && (
            <div className="flex gap-2.5 items-center bg-[#FFF2E8] p-3 rounded-xl border border-[#F7882F]/30 text-slate-800 text-xs shadow-sm">
              ...
              <div className="space-y-0.5">
                ...
                <div className="text-[10px] text-[#6B7A8F]">Synthesizing ranking metrics & policy data</div>
              </div>
            </div>
          )}
